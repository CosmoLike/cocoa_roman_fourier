...
flags:
    ...
    python -m pytest ./projects/roman_fourier/tests --high=1 --mask=ones
...
# the masks the comparison sweeps can run under; "frozen" is the
# roman_example.mask of the frozen contract, and "ones" keeps every
# point (no scale cuts). Each choice is a frozen TATT dataset variant
# differing only in its mask_file line; the mapping lives in the
# shared harness (cocoa_testing.CocoaTestHarness, fastpt_masks).
    ...
    try:
        parser.addoption(
            ...
            help="the scale-cut mask of the CFASTPT-vs-FASTPT "
                 ...
                 "default) or ones (every data point kept)")
    except ValueError:
        pass
